...
\abstract{This report presents a comprehensive study on the hydrodynamic resistance of yachts using Bayesian Learning and Monte Carlo Simulation techniques. The primary goal is to model and predict the resistance forces acting on yacht hulls as they move through water. By applying Bayesian inference methods, we estimate the posterior distributions of resistance coefficients based on observed data and prior knowledge. Monte Carlo simulation is employed to propagate uncertainty through the model, allowing for robust predictions that account for variabilities in environmental conditions and material properties. The results provide insights into the efficiency of different hull designs and contribute to the optimization of yacht performance in marine engineering applications.}
...
The data analysed in this report is the \emph{Yacht Hydrodynamics} dataset
from the UCI Machine Learning Repository
\autocite{ortigosa2007yacht}.

The dataset comprises $22$ different hull geometries, each tested at a
range of speeds (Froude numbers), for a total of $308$ observations
(hull-form $\times$ Froude-number rows). Each observation is described by
five hull-geometry parameters and the Froude number, together with the
measured residuary resistance, which is the target variable. The variables
are summarised in \cref{tab:variables} in the appendix.

The first five covariates (\texttt{LCB}, \texttt{prismatic},
\texttt{L\_disp}, \texttt{beam\_draught}, \texttt{L\_beam}) are geometric
form factors that are constant within a given hull and characterise its
shape. The Froude number is a dimensionless measure of speed and is the
dominant predictor of resistance: for sailing yachts the residuary
resistance is known to scale approximately as a power law of the Froude
number, with an exponent close to $4$ in the bulk regime. This physical
expectation motivates the log--log regression and nonlinear modelling
adopted in the remainder of the report. The target \texttt{resistance}
is strongly right-skewed, ranging over more than three orders of
magnitude, which makes a logarithmic treatment and an explicit account of
heteroscedasticity essential for sound Bayesian inference.
...
The physics of slender sailing hulls suggests that, in the bulk regime,
the residuary resistance grows as a power of the Froude number,
$\mathrm{resistance}\propto Fn^{\,\gamma_F}$ with $\gamma_F$ close to $4$.
Taking logarithms turns this multiplicative law into a linear model, in
which the exponent appears directly as a regression slope. We therefore
fit
\begin{equation}
	...
	\label{eq:loglog}
\end{equation}
where $\mathrm{hull}_k$ denotes the five geometric form factors
(\texttt{LCB}, \texttt{prismatic}, \texttt{L\_disp},
\texttt{beam\_draught}, \texttt{L\_beam}). Since the smallest observed
resistance is $0.01>0$, the logarithm is well defined for every record
and no offset is required. Writing the $n=308$ observations in matrix form
as $\bm{y}=\bm{X}\bm{\beta}+\bm{\varepsilon}$ with $\bm{\beta}\in\mathbb{R}^{p}$,
$p=7$, we adopt the non-informative reference (Jeffreys) prior
\begin{equation}
	p(\bm{\beta},\sigma^2)\;\propto\;\sigma^{-2},
	\label{eq:prior}
\end{equation}
