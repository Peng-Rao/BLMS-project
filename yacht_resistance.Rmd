...
$$ For the heteroscedasticity-treated nonlinear model M1, the predictive distribution is Gamma on the original scale with mean $\mu_i$ and mean-dependent standard deviation $\sigma_i$: $$
y_i^{\mathrm{rep}} \sim \mathrm{Gamma}\left(\alpha_i,\ r_i\right),\qquad
\mu_i=\exp\{b_0 + \gamma_F\widetilde{\log Fn}_i + \beta_B\widetilde{bd}_i\},\qquad
\sigma_i=\sigma_0\,\mu_i^{\delta},
$$ with $\alpha_i=(\mu_i/\sigma_i)^2$ and $r_i=\mu_i/\sigma_i^2$. This keeps replicated resistances positive by construction while still letting the predictive band widen where the resistance, and hence the scatter, is larger. The curves hold hull form fixed at a representative value: `beam_draught` is set to its sample median, while the other log-log hull covariates are set to their sample means. This isolates the Froude-resistance curve rather than mixing different hull geometries.
...
The plot shows the practical consequence of the updated Task 3 model. The log-log fit produces a lognormal posterior predictive band on the original scale: it is positive by construction, narrow near zero resistance, and expands multiplicatively as resistance grows. The Gamma M1 fit also stays on the positive support, but it learns the variance growth directly through $\sigma_i=\sigma_0\mu_i^{\delta}$ with $\delta\approx0.89$. Thus its band is close to multiplicative, but not imposed to be exactly so. This explains why the fitted exponent $\gamma_F\approx4.82$ is close to the log-log estimate $\beta_F\approx4.69$: both models weight the data in a broadly relative-error way.

The main visible difference is therefore not a dramatic change in the mean curve, but a cleaner original-scale uncertainty statement. The Gamma band remains non-negative and adapts its width to the resistance level, while the constant-variance comparison in Part 3 used a single absolute scale and was strongly rejected by WAIC/PSIS-LOO. The updated posterior predictive check therefore reinforces the current Task 3 conclusion: learning heteroscedasticity is essential, but once the positive-support Gamma likelihood is used, the Froude exponent stays near the log-log estimate and the `beam_draught` effect is not credibly resolved.

**Conclusion of part 4.** Both posterior predictive families respect the positivity of resistance, and both show uncertainty increasing with the Froude-driven mean. The log-log model imposes multiplicative uncertainty through the transformation; the Gamma M1 model estimates the variance growth on the original scale. The comparison with the observed data supports the heteroscedastic treatment while also showing that the robust conclusion is the Froude power law itself, not a strong hull-geometry effect.
...
This train/test check is not used to replace the full-data posterior analysis. It is only a posterior predictive sanity check on the split rows. Higher log predictive density is better, while coverage and band width show calibration. The nonlinear scores are now evaluated under the same Gamma likelihood used in Task 3, so the validation is on the same positive-support predictive scale as the posterior predictive curves. The learned-variance Gamma model is preferred when it gives the strongest test-set posterior predictive density and calibrated 95% intervals without simply inflating the bands. This agrees with the Part 4 plot: learning the heteroscedastic variance reallocates uncertainty across the Froude range instead of relying on point predictions from a separate frequentist fit.
